...
public class EventEditActivity extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        if (eventIndex >= 0)
        {
            Event existing = Event.get(eventIndex);
    ...
    public void saveEventAction(View view)
    {
        ...
        if (eventIndex >= 0)
        {
            // Preserve the original ID so the alarm reference stays valid
            int existingId = Event.get(eventIndex).getId();
            EventAlarmScheduler.cancel(this, existingId);
            event = new Event(
                existingId, name,
            ...
            Event.set(eventIndex, event);
        }
        else
        {
            event = new Event(
                name, eventDate, startTime, endTime, selectedColor,
                selectedRecurrenceType(), selectedInterval(),
                reminderMinutes, eventType, importance, difficulty
            );
            Event.add(event);
    ...
    public void deleteEventAction(View view)
    {
        if (eventIndex >= 0)
        {
            EventAlarmScheduler.cancel(this, Event.get(eventIndex).getId());
            Event.remove(eventIndex);
        }
        EventStorage.save(this);
        finish();
    }
